...
export default function ModalTrocarSenha({ onClose, onSuccess, forced = false }) {
  ...
  const handleSalvar = async () => {
    ...
    setLoading(true);
    try {
      const data = await api.trocarSenha(atual, nova);
      setMsg('Senha alterada com sucesso!');
      setErro('');
      onSuccess?.(data);
      if (!onSuccess && onClose) setTimeout(onClose, 1200);
    } catch (e) {
      setErro(e.message);
    } finally {
      setLoading(false);
      ...
      <div style={{ background: '#fff', borderRadius: 18, width: '100%', maxWidth: 420, boxShadow: '0 8px 48px #0f2a5540' }}>
        <div style={{ background: 'linear-gradient(135deg,#1e3a5f,#2d5a9e)', padding: '20px 24px', borderRadius: '18px 18px 0 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ color: '#fff', fontWeight: 700, fontSize: 16 }}>
            {forced ? '🔒 Troca Obrigatória de Senha' : '🔒 Trocar Senha'}
          </div>
          {!forced && (
        ...
        <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 14 }}>
          {forced && (
            <div style={{ background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: 10, padding: '12px 14px', color: '#1d4ed8', fontSize: 13, fontWeight: 600 }}>
              Este acesso está bloqueado até a troca da senha inicial.
            </div>
          )}
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
            {!forced && <Btn variant="secondary" onClick={onClose}>Cancelar</Btn>}
            <Btn onClick={handleSalvar} disabled={loading}>
              {loading ? 'Salvando...' : (forced ? 'Alterar e continuar' : 'Salvar')}
            </Btn>
          </div>
        </div>
      </div>
